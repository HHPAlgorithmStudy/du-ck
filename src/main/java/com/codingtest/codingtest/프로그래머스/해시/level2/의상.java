package com.codingtest.codingtest.프로그래머스.해시.level2;
...
public class 의상 {
    ...
    public static void main(String[] args) {
        ...
        int result = solution.solution2(new String[][]{
                {"yellow_hat", "headgear"},
    ...
    static class Solution {
        ...
        public int solution(String[][] clothes) {
            ...
            //경우의 수 구하기
            // key 는 옷의 종류, value 는 옷 이름 이다.
            // value 들 끼리 곱하면 조합했을 때의 총 경우의 수가 나온다.
            int combinations = 1; // 곱셈이므로 초기값은 1
            for (Integer value : map.values()) {
                combinations *= value;
            }
            //조합했을 때의 총 경우의 수와
            //옷 종류의 개수 (1개씩 입었을때) 를 더한다.
            return combinations;
        }
        ...
        public int solution2(String[][] clothes) {
            ...
            for (int value : map.values()) {
                combinations *= (value + 1); // 해당 종류를 입지 않는 경우 포함
            }
            // 위에서 종류별로 입지않는 경우를 추가하였기 때문에
            // 아무것도 입지 않는 케이스가 추가되므로
            // 해당 경우를 제외
